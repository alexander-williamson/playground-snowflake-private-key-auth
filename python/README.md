...
6. Run `python main.py`. You should see a successful connection and the current Snowflake user printed to the console.

```
python main.py
```

Should output your username after the debug messages:

```bash
DEBUG: env_file=/Users/example/github/playground-snowflake-private-key-auth/python/.env
DEBUG: account=example.eu-west-1
DEBUG: user=example@example.com
DEBUG: private_key_file=/Users/example/github/playground-snowflake-private-key-auth/rsa_key.p8
DEBUG: warehouse='EXAMPLE_WAREHOUSE'
DEBUG: database='EXAMPLE_DATABASE'
DEBUG: schema='EXAMPLE_SCHEMA'
example@example.com
```
